...
export default {
  ...
  methods: {
    ...
    _handleGlobalDeleteByIds (ids, delFunction) {
      this._handleGlobalByIds(ids, delFunction, 'delete')
    },
    _handleGlobalDelete (delFunction) {
      this._handleGlobal(delFunction, 'delete')
    },
    _handleGlobalDeleteAll (delFunction) {
      this._handleGlobalAll(delFunction, 'delete')
    },
    _handleGlobal (optFunction, opt = 'delete'){
    ...
    _handleGlobalByIds (ids, optFunction, opt = 'delete') {
      const optName = optNameMap[opt]
      this.$confirm(`此操作将永久${optName}该数据, 是否继续?`, '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
