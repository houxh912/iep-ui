...
export default {
  ...
  methods: {
    ...
    _loadConfig () {
      return new Promise((resolve) => {
        if (
          !!window.UE && !!window.UEDITOR_CONFIG && Object.keys(window.UEDITOR_CONFIG).length !== 0
        ) {
          resolve()
          return
        }
        ...
        configScript.src = `${this.mixedConfig.UEDITOR_HOME_URL}${this.toolbarsUrl !== '' ? this.toolbarsUrl : (this.toolbarsSimply ? 'neditor.config.simply.js' : 'neditor.config.js')}`
