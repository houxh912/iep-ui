...
<script>
import { getFeeById, putFeeRelation } from '@/api/fams/fee'
import { initForm, dictsMap, rules } from '@/views/fams/wealth/Fee/options.js'
...
export default {
  ...
  methods: {
    ...
    async submitForm () {
      const { data } = await putFeeRelation(this.form)
      if (data.data) {
