  ...
  <div class="title">
        ...
        <div class="ti2">
          <draggable element="div" v-model="data.optionList" :animation="100">
            <div class="mg10" v-for="(item,index) in data.optionList" :key="index">
              <el-input :maxlength="200" style="width:200px;border:none;text-align:center;margin-right:30px" size="mini" placeholder="请输入选项" v-model="item.name"></el-input>
    ...
    <template v-else>
      ...
      <div style="margin:10px 0" v-for="(item,index) in data.optionList" :key="index">
        {{item.name}}
        <el-input style="width:300px" size="mini" v-model="data.answer[item.name]" :disabled="disabled" :maxlength="200"></el-input>
      </div>
    </template>
  </div>
</template>
