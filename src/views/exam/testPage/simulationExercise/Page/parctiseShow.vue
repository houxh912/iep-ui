                ...
                <el-row class="module" :gutter="20">
                  <el-col class="module-item" v-for="(item,index) in testListRes" :key="index"
                    ...
                    <el-card shadow="hover">
                      <div class="header">
                        <img src="../../subjectTest/img/1.jpg" alt="" class="img">
                        <div class="total">
...
<script>
import { getTestOption, getPracticeTestList } from '@/api/exam/testPage/subjectTest/subjectTest'
import mixins from '@/mixins/mixins'
import SignDialog from '../../subjectTest/Page/SignDialog'
import Status from '../../subjectTest/Page/status'
export default {
  mixins: [mixins],
   props: ['record'],
  ...
  methods: {
    ...
    handleSubject (item) {
      ...
      getPracticeTestList({ ...this.pageOption, ...this.choosedPaginationOption }).then(response => {
     ...
     */
    getList () {
      getPracticeTestList({ ...this.pageOption, ...this.paginationOption }).then(response => {
        const { records, size, total, current } = response.data.data
        this.testListRes = records
        this.total = total
        this.paginationOption = {
     ...
     */
    reload () {
      getPracticeTestList({ ...this.reloadPageOption, ...this.reloadPaginationOption }).then(response => {
        const { records, size, total, current } = response.data.data
        this.testListRes = records
        this.total = total
        this.paginationOption = {
