<template>
  <div class="jobmanage">
    <basic-container>
      <avue-crud ref="crud" :page="page" v-model="form" :data="tableData" :table-loading="tableLoading" :option="tableOption" @on-load="getList" @row-save="save" @row-update="update" @refresh-change="refreshChange" @search-change="handleFilter" @search-reset="handleSearchReset">
        ...
        <template slot="menuLeft">
          <iep-button type="primary" @click="handleAdd" size="small" v-if="permissions.job_sys_job_add" icon="el-icon-plus" plain>
            新建任务
          </iep-button>
          ...
          <el-tooltip content="谨慎使用" placement="top">
            <iep-button @click="refreshJobs" size="small" v-if="permissions.job_sys_job_refresh_job">重置全部任务
            </iep-button>
          </el-tooltip>
        </template>
        <template slot="menu" slot-scope="scope">
          <el-button type="text" icon="el-icon-info" size="mini" plain @click="getJobLog(scope.row)">日志
          </el-button>
          <el-button type="text" icon="el-icon-caret-right" size="mini" v-if="permissions.job_sys_job_start_job" plain @click="handleStartJob(scope.row)">启动
          </el-button>
          <el-button type="text" icon="el-icon-error" size="mini" v-if="permissions.job_sys_job_shutdown_job" plain @click="handleShutDownJob(scope.row)">暂停
          </el-button>
          <el-button type="text" icon="el-icon-edit" size="mini" v-if="permissions.job_sys_job_edit" plain @click="handleUpdate(scope.row,scope.index)">修改
          </el-button>
          <el-button type="text" v-if="permissions.job_sys_job_del" icon="el-icon-delete" size="mini" plain @click="handleDelete(scope.row,scope.index)">删除
          </el-button>
        </template>
      </avue-crud>
    </basic-container>
    <el-dialog title="执行日志" :visible.sync="dialogFormVisible" width="90%" @close="closeJobLogDialog">
      <avue-crud ref="crudLog" :page="pageLog" @on-load="getJobLogList" :data="tableLogData" :option="tableLogOption" @refresh-change="getJobLogList" :table-loading="tableLogLoading">
