    ...
    <basic-container>
      ...
      <operation-container>
        ...
        <template slot="right">
          ...
          <operation-search @search-page="searchPage" advance-search></operation-search>
        </template>
      </operation-container>
...
export default {
  ...
  methods: {
    ...
    handleDelete (row) {
      this._handleGlobalDeleteById(row.id, deleteCustomerById)
    },
    loadPage (param = { ...this.searchForm, type: this.type }) {
      this.loadTable(param, getCustomerPage)
    },
  },
}
</script>
